...
app.config['SECRET_KEY'] = os.environ.get(
    'FLASK_SECRET_KEY', 'default-flask-secret-key')
...
class Usuario(db.Model, UserMixin):
    ...
    # AUMENTANDO O TAMANHO DO CAMPO DA SENHA
    password_hash = db.Column(db.String(256), nullable=False)

    demandas_criadas = db.relationship('Demanda', backref='criador', lazy=True)
    equipamentos_criados = db.relationship(
        'Equipamento', backref='criador', lazy=True)
...
class PrioridadeDemanda(enum.Enum):
    BAIXA = "Baixa"
    MEDIA = "Média"
    ALTA = "Alta"
...
class Equipamento(db.Model):
    ...
    status = db.Column(db.Enum(StatusEquipamento),
                       default=StatusEquipamento.DISPONIVEL, nullable=False)
...
class Demanda(db.Model):
    ...
    status = db.Column(db.Enum(StatusDemanda),
                       default=StatusDemanda.A_FAZER, nullable=False)
    prioridade = db.Column(db.Enum(PrioridadeDemanda),
                           default=PrioridadeDemanda.MEDIA, nullable=False)
    data_criacao = db.Column(
        db.DateTime, nullable=False, default=datetime.utcnow)
    user_id = db.Column(db.Integer, db.ForeignKey(
        'usuario.id'), nullable=False)
...
def gerenciar_equipamentos():
    if request.method == 'POST':
        novo_equipamento = Equipamento(
            modelo=request.form['modelo'], loja=request.form['loja'], criador=current_user)
    ...
    equipamentos = Equipamento.query.order_by(
        Equipamento.data_criacao.desc()).all()
...
def gerenciar_demandas():
    if request.method == 'POST':
        nova_demanda = Demanda(
            descricao=request.form['descricao'], prioridade=PrioridadeDemanda[request.form['prioridade']], criador=current_user)
...
def atualizar_status_demanda(id):
    ...
    demanda.status = StatusDemanda[request.form['status']]
    db.session.commit()
    return redirect(url_for('gerenciar_demandas'))
...
def apagar_demanda(id):
    ...
    db.session.commit()
    flash('Demanda apagada com sucesso!', 'success')
    return redirect(url_for('gerenciar_demandas'))
...
def editar_equipamento(id):
    ...
    if request.method == 'POST':
        ...
        equipamento.loja = request.form['loja']
        db.session.commit()
...
def apagar_equipamento(id):
    ...
    db.session.commit()
    flash('Equipamento apagado com sucesso!', 'success')
    return redirect(url_for('gerenciar_equipamentos'))
...
def gerenciar_credenciais():
    if request.method == 'POST':
        ...
        nova_credencial = Credencial(
            servico=request.form['servico'], login=request.form['login'], senha_criptografada=senha_criptografada.decode('utf-8'))
    ...
    credenciais = Credencial.query.all()
    return render_template('credenciais.html', credenciais=credenciais)
...
def visualizar_credencial(id):
    ...
    try:
        senha_pura = fernet.decrypt(
            credencial.senha_criptografada.encode('utf-8')).decode('utf-8')
...
def apagar_credencial(id):
    ...
    db.session.commit()
    flash('Credencial apagada com sucesso!', 'success')
    return redirect(url_for('gerenciar_credenciais'))
